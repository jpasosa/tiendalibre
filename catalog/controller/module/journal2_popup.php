...
class ControllerModuleJournal2Popup extends Controller {
    ...
    public function show() {
    	if (isset($this->request->get['privacy'])) {
			if ($this->request->get['privacy']) {
				$this->load->model('catalog/information');
				$this->load->language('account/register');

				$information_id = (int)$this->request->get['privacy'];
				...
				if ($information_info) {
					$information = array(
						...
						'information_error' => sprintf($this->language->get('error_agree'), $information_info['title'])
					);
				} else {
					$information = null;
				}
    ...
    private function validate() {
		if ($this->journal2->settings->get('popup_privacy_information') && (!isset($this->request->post['agree']) || !$this->request->post['agree'])) {
			$this->contact_error['agree'] = $this->journal2->settings->get('popup_privacy_information.information_error');
		}
